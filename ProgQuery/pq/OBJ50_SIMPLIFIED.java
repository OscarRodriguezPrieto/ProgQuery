...
public class OBJ50_SIMPLIFIED extends AbstractQuery {
	...
	protected void initiate() {
		clauses = new Clause[] {
				...
				new WhereClause(
						"NOT declaration:THIS_REF AND NOT mutatorExpr:INITIALIZATION AND (r.isInit IS NULL OR NOT r.isInit) "),
				...
				new MatchClause(new MatchImpl(getStatementServices()
						.getEnclosingClassFromStatement(
								getExpressionServices().getStatementFromExp(new NodeVar("mutatorExpr")))
//						.append(Pair.create(
//								new EdgeImpl(EdgeDirection.OUTGOING, RelationTypes.HAS_INDIRECT_TYPE_DEC,
						// RelationTypes.HAS_TYPE_DEC),
						// new NodeVar("mutatorExprCu")))
						.matchToString().replace("enclClass", "exprEnclClass"))),
