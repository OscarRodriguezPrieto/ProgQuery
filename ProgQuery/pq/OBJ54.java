...
public class OBJ54 extends AbstractQuery {
	...
	protected void initiate() {
		clauses = new Clause[] {
				...
				new WhereClause("varDec:VAR_DEC OR varDec:PARAMETER_DEC"),
				...
				new MatchClause(true, getStatementServices().getEnclosingClassFromDeclaration(new NodeVar("varDec"))),
				new ReturnClause(
						// "varDec, assStat, useStats, enclClass"
						"'Warning [CMU-OBJ54] You must not try to help garbage collector setting references to null when they are no longer used. To make your code clearer, just delete the assignment in line ' + assStat.lineNumber + ' of the variable ' +varDec.name+ ' declared in class '+enclClass.fullyQualifiedName+'.'"
						)
				};
	}
