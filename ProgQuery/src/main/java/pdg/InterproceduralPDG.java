...
//				"INTERPROCEDURAL FLOW TRAVERSING :\n" + methodInfo.methodNode.getProperty("fullyQualifiedName"));
		...
		// SE RECUPERAN LOS CALLS DE CADA MÉTODO
		if (methodDecToCalls.containsKey(methodDec))
			...
			for (RelationshipWrapper callRel : methodDecToCalls.get(methodDec)) {
				...
				// ITERAMOS SOBRE LAS POSIBLES DEFINICIONES DE LOS MÉTODOS LLAMADOS
				for (RelationshipWrapper invocationReferringDec : possibleMethodDecsForCalls) {
					...
					if (calledMethodInfo != null)
						// ITERAMOS SOBRE CADA UNO DE LOS PARÁMETROS (INCLUIDOS THIS -0-) cuyo estado se
						// modifica (o puede) durante la ejecución del método llamado
...
//						"DEC NODE TO LINK TO INVS " + NodeUtils.nodeToString(decToInvPDGRel.getKey().getFirst()));
	 ...
	 * para todos los MAY_REFER_TO si hay una relación s-r->e para todas entonces el
	 * tipo es r si sólo hay en algunas s-r->e , el tipo es MAY_
	 */
...
//				+ methodInfo.methodNode.getProperty("fullyQualifiedName"));
		...
		// invocación : invocationModifyVarsInfo asocia el index de cada arg (el objeto
